...
- **Current state:** prototype; `veil` (Core) ships as an RPM with profile intent
  manifests and experience catalogs; profiles recommend, `veil profile sync`
  installs experiences as DNF meta-packages
  (see `docs/day3-intent-engine.md`)
...
- See `docs/day3-intent-engine.md` for the current prototype: what is
...
scripts/smoke-day3.sh      # 62 checks, run on clean state
```
...
(see `docs/adr/0004-profile-baseline-not-prison.md`).
